function [time_, freq_, mdata_] = ir_join_wz(time1, freq1, mdata1, weight1, time, freq2, mdata2, weight2, freq)
  ...
  if(min(freq1)<min(freq2) && max(freq1)<max(freq2))
    ...
    overlap_data = (mdata1(overlap_idx1:end,:)*weight1+mdata2_overlapp_comp*weight2)/(weight1+weight2);
  ...
  elseif(min(freq1)>min(freq2) && max(freq1)>max(freq2))
    ...
    overlap_data = (mdata1(1:overlap_idx1,:)*weight1+mdata2_overlapp_comp*weight2)/(weight1+weight2);
